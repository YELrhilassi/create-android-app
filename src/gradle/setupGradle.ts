...
export async function setupGradle(projectPath: string) {
...
async function ensureWrapperJar(destPath: string) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Failed to fetch wrapper jar: ${response.statusText}`);
